...
check (min, max, letterAsString, passwd) = 
    let 
        letter = head letterAsString
        occur  = occurences letter passwd
    in 
        occur >= min && occur <= max 

checkV2 :: (Int, Int, [Char], String) -> Bool
checkV2 (min, max, letterAsString, passwd) = 
    let 
        letter = head letterAsString
        l1 = passwd !! (min - 1)
        l2 = passwd !! (max - 1)
    in 
        (l1 == letter) /= (l2 == letter)  -- xor

occurences :: Char -> String -> Int
occurences letter = length . filter (== letter) 
...
-- (min occurence, max occurences, letter, passwd)
input :: [(Int, Int, [Char], String)]
input = [ (2, 8, "t", "pncmjxlvckfbtrjh"),
            (8, 9, "l", "lzllllldsl"),
